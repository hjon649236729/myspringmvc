...
<head>
    ...
    <script type="text/javascript">
        ...
        function showtask(showurl) {
            $(document).stk_go(sirm.jsurl(showurl), "查看任务", {keyWithoutParams:true});
        }

        function recoverProcess(exampleId, exampleStepId) {
            var confirmMsg = "确定要撤回此流程吗";
            stk.confirm(confirmMsg, function () {
                var params = {exampleId: exampleId, exampleStepId: exampleStepId};
                stk.call({className: 'com.sinitek.sirm.web.workflow.applicate.ManagerTaskHistoryAction',
                    method: 'recoverProcess', param: params, callBack: function (result) {
        ...
        function changeprocessname_old(value, num) {
...
//            loadbody();
        ...
        function dealTask(data){
            stk.call({
                className:'com.sinitek.sirm.web.workflow.applicate.ManagerTaskAction',
                method:'judgetask',
                param:{exampleownerid: data.exampleownerid},
                callBack:function(result) {
                    if (result != null) {
                        stk.alert(result, "提示");
                        $("#unfinishtable").stk_query();
...
//                        $(document).stk_go(sirm.jsurl(actionurl + "?exampleid=" + exampleid + "&examplestepid=" + examplestepid + "&exampleownerid=" + exampleownerid), "处理任务", {keyWithoutParams:true});
                        sirm.openTab(sirm.jsurl(data.actionurl2), "处理任务", {keyWithoutParams: true}, function () {
                            $("#unfinishtable").stk_query();
                            $("#finishedtable").stk_query();
                        });
                    } else {
...
<layout:tab title="待处理" id="unfinishtab">
    <form id="unfinishform">
        ...
        <plugin:colLink id="condition2" dataVar="data" text="处理">
            ...
            <%--$(document).stk_closeMask();--%>
            <%--if(data.actionurl!=null&&data.actionurl.length>0){--%>
            <%--&lt;%&ndash;$(document).stk_go(data.actionurl2,"处理任务",{keyWithoutParams:true});&ndash;%&gt;--%>
        ...
        <ui:table id="unfinishtable" queryForm="unfinishform" nowrap="true" allowInit="true"
                  actionClass="com.sinitek.sirm.web.workflow.applicate.ManagerTaskAction"
                  pageSize="10">
            <%--<ui:col plugin="checkBoxPlugin" show="false" width="0px" align="center"/>--%>
            <ui:col property="processname" title="流程名称" align="left" width="150px" allowSort="false"></ui:col>
