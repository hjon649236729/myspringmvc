...
public class BaseService<T> extends BaseDao<T> {
	...
	public List<T> getAll(CriterionCollection criterionCollection,
			Order... orders) {
	...
	public Page pagedQuery(String sql, int pageNo, int pageSize,
		...
		return super.pagedQuery(sql, pageNo, pageSize, params);
	}
	...
	// (BasicDataSource)springContextUtil.getBean("dataSource");
	// Constant.dbType =
	// dataSource.getUrl().split("://")[0].split(":")[1].toLowerCase();
	// }
